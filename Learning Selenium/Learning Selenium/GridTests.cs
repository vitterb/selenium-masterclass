...
{
    internal class GridTest : BaseTest
    {
        private OrderPage orderPage;

        // before running test run "java -jar selenium-server-4.13.0.jar hub" (in first command prompt window)
        // Then Open nodes "java -jar ./selenium-server-4.13.0.jar node --port 5555" (in second command prompt window)
        // Finally open nodes "java -jar selenium-server-4.13.0.jar node --port 6666" (in third command prompt window)

        [SetUp]
        public override void Setup()
        {
            var options = new ChromeOptions();
            SetDriver(new RemoteWebDriver(new Uri("http://localhost:4444/wd/hub"), options));
